...
class Test_auth(BaseTestCase):
    def test_successful_signup(self):
        ...
        """
        with self.client:
            response = self.register_user("huzaifah", "huz@gmail.com", "12345","True")
            data = json.loads(response.data.decode())
            self.assertEqual(response.status_code, 201)
            self.assertEqual(data.get('message'), "User successfully created")
            # Add the same user and see...
            res = self.register_user("huzaifah", "huz@gmail.com", "12345","True")
    ...
    def test_username_with_characters_onsignup(self):
        ...
        with self.client:
            response = self.register_user("%2?1@", "ui@gmail.com", "12345","True")
            data = json.loads(response.data.decode())
            self.assertEqual(response.status_code, 400)
